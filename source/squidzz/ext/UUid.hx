...
class Uuid {
	...
	static var rndSeed:Int64 = Int64.fromFloat(#if js js.lib.Date.now() #elseif sys Date.now().getTime() * 1000 #else Date.now().getTime() #end);
	...
	public static function v1(node:Bytes = null, optClockSequence:Int = -1, msecs:Float = -1, optNsecs:Int = -1, ?randomFunc:Void->Int,
		...
		if (msecs == -1) {
			msecs = Math.fround(#if js js.lib.Date.now() #elseif sys Date.now().getTime() * 1000 #else Date.now().getTime() #end);
		}
		var nsecs = optNsecs;
		if (optNsecs == -1) {
			nsecs = lastNSecs + 1;
